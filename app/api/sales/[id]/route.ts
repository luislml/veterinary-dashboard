...
export async function GET(
    ...
    try {
        ...
        const response = await fetch(`${API_CONFIG.baseURL}/sales/${id}`, {
            method: 'GET',
            headers,
        });
        ...
        if (!response.ok) {
            return NextResponse.json(
                { error: data.message || 'Venta no encontrada' },
...
export async function PUT(
    ...
    try {
        ...
        if (!response.ok) {
            return NextResponse.json(
                { error: data.message || 'Error al actualizar venta', errors: data.errors },
                { status: response.status }
            );
        }
