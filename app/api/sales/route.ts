...
/**
 * GET /api/sales - Obtener lista de ventas
...
export async function GET(req: NextRequest) {
    try {
        ...
        const laravelUrl = new URL(`${API_CONFIG.baseURL}/sales`);
        ...
        if (!response.ok) {
            return NextResponse.json(
                { error: data.message || 'Error al obtener ventas' },
...
export async function POST(req: NextRequest) {
    try {
        ...
        if (!response.ok) {
            return NextResponse.json(
                { error: data.message || 'Error al crear venta', errors: data.errors },
                { status: response.status }
            );
        }
