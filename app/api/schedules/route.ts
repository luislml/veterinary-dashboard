...
/**
 * GET /api/schedules - Obtener lista de horarios
...
export async function GET(req: NextRequest) {
    try {
        ...
        const laravelUrl = new URL(`${API_CONFIG.baseURL}/schedules`);
        ...
        if (!response.ok) {
            return NextResponse.json(
                { error: data.message || 'Error al obtener horarios' },
...
export async function POST(req: NextRequest) {
    try {
        ...
        if (!response.ok) {
            return NextResponse.json(
                { error: data.message || 'Error al crear horario', errors: data.errors },
                { status: response.status }
            );
        }
