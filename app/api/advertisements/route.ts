...
/**
 * GET /api/advertisements - Obtener lista de anuncios
...
export async function GET(req: NextRequest) {
    try {
        ...
        const laravelUrl = new URL(`${API_CONFIG.baseURL}/advertisements`);
        ...
        if (search) {
            laravelUrl.searchParams.append('search', search);
        }
        ...
        if (!response.ok) {
            return NextResponse.json(
                { error: data.message || 'Error al obtener anuncios' },
        ...
        console.error('Error en GET /api/advertisements:', error);
...
/**
 * POST /api/advertisements - Crear un nuevo anuncio
 * Proxy a la API de Laravel
 */
export async function POST(req: NextRequest) {
    try {
        ...
        const response = await fetch(`${API_CONFIG.baseURL}/advertisements`, {
            method: 'POST',
            headers,
        ...
        if (!response.ok) {
            return NextResponse.json(
                { error: data.message || 'Error al crear anuncio', errors: data.errors },
                { status: response.status }
            );
        }
        ...
        console.error('Error en POST /api/advertisements:', error);
        return NextResponse.json(
            { error: 'Error interno del servidor' },
            { status: 500 }
        );
