...
/**
 * GET /api/shoppings - Obtener lista de compras
...
export async function GET(req: NextRequest) {
    try {
        ...
        const laravelUrl = new URL(`${API_CONFIG.baseURL}/shoppings`);
        ...
        if (!response.ok) {
            return NextResponse.json(
                { error: data.message || 'Error al obtener compras' },
        ...
        console.error('Error en GET /api/shoppings:', error);
...
/**
 * POST /api/shoppings - Crear una nueva compra
 * Proxy a la API de Laravel
 */
export async function POST(req: NextRequest) {
    try {
        ...
        const response = await fetch(`${API_CONFIG.baseURL}/shoppings`, {
            method: 'POST',
            headers,
        ...
        if (!response.ok) {
            return NextResponse.json(
                { error: data.message || 'Error al crear compra', errors: data.errors },
                { status: response.status }
            );
        }
        ...
        console.error('Error en POST /api/shoppings:', error);
        return NextResponse.json(
            { error: 'Error interno del servidor' },
            { status: 500 }
        );
