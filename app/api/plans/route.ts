...
/**
 * GET /api/plans - Obtener lista de planes
...
export async function GET(req: NextRequest) {
    try {
        ...
        const laravelUrl = new URL(`${API_CONFIG.baseURL}/plans`);
        ...
        if (!response.ok) {
            return NextResponse.json(
                { error: data.message || 'Error al obtener planes' },
...
export async function POST(req: NextRequest) {
    try {
        ...
        const response = await fetch(`${API_CONFIG.baseURL}/plans`, {
            method: 'POST',
            headers,
        ...
        if (!response.ok) {
            return NextResponse.json(
                { error: data.message || 'Error al crear plan', errors: data.errors },
                { status: response.status }
            );
        }
        ...
        console.error('Error en POST /api/plans:', error);
        return NextResponse.json(
            { error: 'Error interno del servidor' },
            { status: 500 }
        );
