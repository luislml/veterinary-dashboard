...
/**
 * GET /api/races - Obtener lista de razas
...
export async function GET(req: NextRequest) {
    try {
        ...
        if (!response.ok) {
            return NextResponse.json(
                { error: data.message || 'Error al obtener razas' },
...
/**
 * POST /api/races - Crear una nueva raza
 * Proxy a la API de Laravel
 */
export async function POST(req: NextRequest) {
    try {
        ...
        const response = await fetch(`${API_CONFIG.baseURL}/races`, {
            method: 'POST',
            headers,
        ...
        if (!response.ok) {
            return NextResponse.json(
                { error: data.message || 'Error al crear raza', errors: data.errors },
                { status: response.status }
            );
        }
        ...
        console.error('Error en POST /api/races:', error);
        return NextResponse.json(
            { error: 'Error interno del servidor' },
            { status: 500 }
        );
