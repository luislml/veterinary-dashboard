...
/**
 * GET /api/type-pets - Obtener lista de tipos de mascotas
...
export async function GET(req: NextRequest) {
    try {
        ...
        const laravelUrl = new URL(`${API_CONFIG.baseURL}/type-pets`);
        ...
        if (!response.ok) {
            return NextResponse.json(
                { error: data.message || 'Error al obtener tipos de mascotas' },
        ...
        console.error('Error en GET /api/type-pets:', error);
...
/**
 * POST /api/type-pets - Crear un nuevo tipo de mascota
 * Proxy a la API de Laravel
 */
export async function POST(req: NextRequest) {
    try {
        ...
        const response = await fetch(`${API_CONFIG.baseURL}/type-pets`, {
            method: 'POST',
            headers,
        ...
        if (!response.ok) {
            return NextResponse.json(
                { error: data.message || 'Error al crear tipo de mascota', errors: data.errors },
                { status: response.status }
            );
        }
        ...
        console.error('Error en POST /api/type-pets:', error);
        return NextResponse.json(
            { error: 'Error interno del servidor' },
            { status: 500 }
        );
